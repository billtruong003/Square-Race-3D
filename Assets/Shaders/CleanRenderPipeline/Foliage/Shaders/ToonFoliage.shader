Shader "CleanRender/ToonFoliage"
{
    Properties
    {
        ...
        _WindStrength("Wind Strength", Range(0, 2)) = 0.5[Toggle(_USE_VERTEX_COLOR_WIND)] _UseVertexColorWind("Vertex Color masks wind (R)", Float) = 1[Toggle(_STATIC_BATCH)] _StaticBatch("Static Batch (disable wind)", Float) = 0
    ...
    SubShader
    {
        ...
        TEXTURE2D(_WindTex);    SAMPLER(sampler_WindTex);float3 ApplyFoliageWind(float3 posWS, float windMask)
        {
            #ifdef _STATIC_BATCH
                return posWS;
            #endif
        ...
        Pass
        {
            ...
            half4 FoliageFrag(Varyings input) : SV_Target
            {
                ...
                half3 litColor = albedo.rgb * lerp(_ShadowColor.rgb, mainLight.color, intensity);
                    ...
                    float subsurface = saturate(dot(-N, mainLight.direction)) * _SubsurfaceStrength * shadow;
