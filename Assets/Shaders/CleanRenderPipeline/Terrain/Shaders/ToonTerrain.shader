Shader "CleanRender/ToonTerrain"
{
    Properties
    {
        ...
        [Header(Hole Map)][Toggle(_USE_HOLEMAP)] _UseHoleMap("Use Hole Map", Float) = 0
    ...
    SubShader
    {
        ...
        {
            half totalSum = dot(w, half4(1.0h, 1.0h, 1.0h, 1.0h));
            return w * rcp(max(totalSum, 1e-4h));
        }
        ...
        Pass
        {
            ...
            half4 TerrainFrag(Varyings input) : SV_Target
            {
                ...
                half cliffMask = 1.0h - saturate((N.y - _CliffAngle) * rcp(max(1.0h - _CliffAngle + 0.01h, 1e-4h)));
                half splatCliffOverride = 0.0h;
                #ifdef _USE_SPLATMAP
                {
                    float2 splatUV = TRANSFORM_TEX(input.uv, _SplatMap);
                    half4 splat = SAMPLE_TEXTURE2D(_SplatMap, sampler_SplatMap, splatUV);
                    half splatSum = dot(splat, half4(1.0h, 1.0h, 1.0h, 1.0h));
                    splat *= rcp(max(splatSum, 1e-4h));
                    half4 sharpSplat = pow(splat, _SplatSharpness);
                    half sharpSum = dot(sharpSplat, half4(1.0h, 1.0h, 1.0h, 1.0h));
                    sharpSplat *= rcp(max(sharpSum, 1e-4h));
                ...
                half flatScale = nonCliffBudget * rcp(max(flatSum, 1e-4h));
                half4 weights = NormalizeWeights(half4(w0 * flatScale, w1 * flatScale, w2 * flatScale, cliffMask));
